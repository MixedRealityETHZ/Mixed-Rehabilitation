...
namespace Microsoft.MixedReality.SceneUnderstanding.Samples.Unity
{
    ...
    public class SceneUnderstandingObjectPlacer : MonoBehaviour
    {
        ...
        {
            objToPlace = Instantiate<GameObject>(objToPlaceRef, UnityEngine.Vector3.zero, UnityEngine.Quaternion.identity);
        ...
        private void UpdateObjPos()
        {
            ...
            UnityEngine.Vector3 newObjPosition = GetDesiredObjPos(UseSUSDKPlacementAPI);
            objToPlace.transform.position = newObjPosition;
        }

        private UnityEngine.Vector3 GetDesiredObjPos(bool useCenterMostPlacementAPI = false)
        {
            RaycastHit hit;
            bool hasTarget = Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit);

            UnityEngine.Vector3 newObjPos = UnityEngine.Vector3.zero;
            if(hasTarget)
            {
                if(useCenterMostPlacementAPI)
                {
                    // Use the SU SDK placement API to get a valid position
                    UnityEngine.Vector3? result = GetPositionFromSUSDKAPI(hit.transform);

                    if(result != null)
                    {
                        UnityEngine.Vector3 selectedObjFacingTowards = -hit.transform.forward.normalized;
                        newObjPos = UnityEngine.Vector3.Dot(Camera.main.transform.TransformDirection(UnityEngine.Vector3.forward), selectedObjFacingTowards) < 0 ? result.Value + (selectedObjFacingTowards * 0.3f) : result.Value - (selectedObjFacingTowards * 0.3f);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    UnityEngine.Vector3 selectedObjFacingTowards = -hit.transform.forward.normalized;
                    newObjPos = UnityEngine.Vector3.Dot(Camera.main.transform.TransformDirection(UnityEngine.Vector3.forward), selectedObjFacingTowards) < 0 ? hit.point + (selectedObjFacingTowards * 0.3f) : hit.point - (selectedObjFacingTowards * 0.3f);
                }
            }
            else
            {
        ...
        IEnumerator SprayCoroutine()
        {
            ...
            for(int i=0; i<10; i++)
            {
                ...
                tempgbj.transform.localScale = new UnityEngine.Vector3(0.2f,0.2f,0.2f);
        ...
        private UnityEngine.Vector3? GetPositionFromSUSDKAPI(Transform obj)
        {
            ...
            if (largestQuad.FindCentermostPlacement(objectExtent, out result))
            {
                ...
                location.transform.localPosition = UnityEngine.Vector3.zero;
                location.transform.localRotation = UnityEngine.Quaternion.identity;
                ...
                UnityEngine.Vector3 resultPos = location.transform.position;
